...
LATEST_REMOTE_TAG="v$(git -c 'versionsort.suffix=-' ls-remote --tags --refs --sort='v:refname' https://github.com/empowerists/bw_web_builds.git 'v*' | tail -n1 | grep -Eo '[^\/v]*$')"
...
${CONTAINER_CMD} create --name "bw_web_${RELEASE_TAG}" "ghcr.io/empowerists/bw_web_builds:${RELEASE_TAG}"
