...
if [ ! -d "${VAULT_FOLDER}" ]; then
    ...
    pushd "${VAULT_FOLDER}"
        ...
        git remote add vaultwarden https://github.com/empowerists/vw_web_builds.git
    popd
else
	...
	then
           ...
           git remote add vaultwarden https://github.com/empowerists/vw_web_builds.git
        elif [ "${VAULTWARDEN_REMOTE}" != "https://github.com/empowerists/vw_web_builds.git" ]
	then
            echo "Warning: \`git remote get vaultwarden\` did not return the expected repository"
	    echo "expected: https://github.com/empowerists/vw_web_builds.git"
